...
app.get('/boja', async (req, res) => {
  try {
    ...
    if (ultimoBoja) {
      ...
      res.send('Ultimo BOJA:' + ultimoBoja.boja + " __ " + ultimoBoja.fechaTxt);
    } else {
...
/*
// CRON tarea 1 05:55:00 
cron.schedule('55 4 * * *', function() {
  (async () => {
    try {
      const res = await fetch('https://nombramientos-sae-herramientassae.replit.app/boja');// SUSTITUIR POR VARIABLE .ENV
